...
public class LoggerBungee {
    private static String prefix = StringUtils.color("&8&l[&b&lDiscord OAuth&8&l] &r");

    private static void log(String message, Type type, boolean prefix) {
        ProxyServer.getInstance().getLogger().info(StringUtils.color((prefix ? LoggerBungee.prefix : "") + (type.getPrefix()) + message));
    }

    private static void log(String prefix, String message, Type type) {
        ProxyServer.getInstance().getLogger().info(StringUtils.color((prefix == null ? "" : prefix) + (type.getPrefix() + message)));
    }
